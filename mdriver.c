...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    int result;
    result = fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
    result = fscanf(tracefile, "%d", &(trace->num_ids));     
    result = fscanf(tracefile, "%d", &(trace->num_ops));     
    result = fscanf(tracefile, "%d", &(trace->weight));        /* not used */
	...
	switch(type[0]) {
	case 'a':
	    result = fscanf(tracefile, "%u %u", &index, &size);
	...
	case 'r':
	    ...
	    break;
	case 'f':
	    result = fscanf(tracefile, "%ud", &index);
